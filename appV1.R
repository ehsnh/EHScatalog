...
server <- function(input, output, session) {
  ...
  observeEvent(input$p_save, {
    ...
    dbExecute(db,
      ...
      list(trimws(input$p_first), trimws(input$p_last),
           input$p_dob %||% NA, input$p_dod %||% NA,
           input$p_nation %||% NA, input$p_bio %||% NA)
    )
  ...
  observeEvent(input$c_save, {
    ...
    dbExecute(db,
      "INSERT INTO collection (name, description, start_date, end_date, curator_id)
       VALUES (?, ?, ?, ?, ?)",
      list(trimws(input$c_name), input$c_desc %||% NA,
           input$c_start %||% NA, input$c_end %||% NA, curator)
    )
  ...
  observeEvent(input$i_save, {
    ...
    dbExecute(db,
      "INSERT INTO item (collection_id, accession_number, title, description, category,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      list(as.integer(input$i_collection),
           trimws(input$i_accession), trimws(input$i_title),
           input$i_desc %||% NA, input$i_category %||% NA,
           input$i_date_created %||% NA, input$i_date_acquired %||% NA,
           input$i_provenance %||% NA, input$i_condition %||% NA,
           input$i_location %||% NA, as.integer(input$i_display))
    )

    # Get the new item's id and save item_person links
    new_id <- dbGetQuery(db, "SELECT last_insert_rowid() AS id")$id
    if (!is.null(input$i_people) && length(input$i_people) > 0) {
      ...
      for (pid in input$i_people) {
        dbExecute(db,
          ...
          list(new_id, as.integer(pid), role))
      }
    }
  ...
  output$item_table <- renderDT({
    refresh_items()
    dbGetQuery(db,
      "SELECT i.item_id AS ID, i.accession_number AS Accession,
  ...
  observeEvent(input$img_save, {
    ...
    dbExecute(db,
      ...
      list(as.integer(input$img_item), filename,
           input$img_caption %||% NA, as.integer(isTRUE(input$img_primary)))
    )
